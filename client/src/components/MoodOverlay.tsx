...
const moods: { id: Mood; label: string; icon: React.ElementType; color: string }[] = [
  { id: 'determined', label: 'Déterminé', icon: Zap, color: 'bg-amber-500' },
  { id: 'happy', label: 'Motivé', icon: Smile, color: 'bg-emerald-500' },
  { id: 'zen', label: 'Zen', icon: Coffee, color: 'bg-blue-500' },
  { id: 'tired', label: 'Fatigué', icon: CloudRain, color: 'bg-slate-500' },
  { id: 'frustrated', label: 'Frustré', icon: Frown, color: 'bg-rose-500' },
];
              ...
              {moods.map((mood, index) => (
                ...
                >
                  <div className={`p-4 rounded-full ${mood.color} bg-opacity-10 group-hover:bg-opacity-20 transition-all`}>
                    <mood.icon className={`w-8 h-8 ${mood.color.replace('bg-', 'text-')}`} />
                  </div>
                  <span className="font-medium text-sm text-foreground/80">{mood.label}</span>
                </motion.button>
              ))}
            </div>
          </div>
