...
export function FloatingJournalButton({ onClick }: FloatingJournalButtonProps) {
  ...
  return (
    <button
      ...
      className="fixed right-6 z-40 p-4 rounded-full shadow-2xl transition-all active:scale-95 bg-gradient-to-br from-purple-500 via-pink-500 to-orange-500 text-white"
      style={{
        // ✅ FIX : bottom = hauteur nav (80px) + safe area + marge (12px)
      ...
      {isPulsing && (
        <>
          ...
          <div
            ...
            style={{ animation: 'ping-slow 3s ease-in-out infinite', animationDelay: '0.5s' }}
          />
        </>
      )}
